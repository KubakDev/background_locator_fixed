...
class IsolateHolderService : MethodChannel.MethodCallHandler, LocationUpdateListener, Service() {
    companion object {
        @JvmStatic val ACTION_SHUTDOWN = "SHUTDOWN"

        @JvmStatic val ACTION_START = "START"

        @JvmStatic val ACTION_UPDATE_NOTIFICATION = "UPDATE_NOTIFICATION"

        @JvmStatic private val WAKELOCK_TAG = "IsolateHolderService::WAKE_LOCK"

        @JvmStatic var backgroundEngine: FlutterEngine? = null

        @JvmStatic private val notificationId = 1

        @JvmStatic var isServiceRunning = false

        @JvmStatic var isServiceInitialized = false
    ...
    private fun start() {
        ...
        Log.d("plugin", "init start waiting 2sec")
        Handler(Looper.getMainLooper())
                .postDelayed(
                        {
                            val notification = getNotification()
                            startForeground(notificationId, notification)
                            Log.d("plugin", "init start waited 2sec")
                            pluggables.forEach { context?.let { it1 -> it.onServiceStart(it1) } }
                            Log.d("plugin", "init start called starts")
                        },
                        2000
                )
    }
    ...
    private fun getNotification(): Notification {
        ...
        return NotificationCompat.Builder(this, Keys.CHANNEL_ID)
                ...
                .setOnlyAlertOnce(
                        true
                ) // so when data is updated don't make sound and alert in android 8.0+
                .setOngoing(true)
                .build()
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when {
            ...
            ACTION_UPDATE_NOTIFICATION == intent?.action -> {
                ...
                synchronized(serviceStarted) { updateNotification(intent) }
            }
        }
        ...
        try {
            ...
            if (location != null) {
                ...
                val result: HashMap<Any, Any> =
                        hashMapOf(Keys.ARG_CALLBACK to callback, Keys.ARG_LOCATION to location)
        ...
        if (backgroundEngine != null) {
            context?.let {
                val backgroundChannel =
                        MethodChannel(getBinaryMessenger(it)!!, Keys.BACKGROUND_CHANNEL_ID)
